...
edges for the labels on a plotted graph.  This can easily be done with
just a couple of extra steps, utilizing the \Rfunction{edgeNames}
function and \Rfunction{unlist} to format the output of
\Rfunction{edgeWeights} appropriately for the attribute list.

<<edgeWeightLabels, fig=TRUE>>=
ew <- edgeWeights(g1)
lw <- unlist(unlist(ew))
names(lw) <- edgeNames(g)
eAttrs$label <- lw
plot(g1, nodeAttrs=nAttrs, edgeAttrs=eAttrs)
@
