...
Note that this is graph has undirected edges, and due to the way
\Rpackage{Rgraphviz} works and the way we went about plotting this,
each single edge is being plotted as two separate undirected edges.
The reason for this is that the \Robject{graphNEL} class notes an edge
from node X to node Y and vice versa in the case of an undirected edge
between nodes X and Y, and \Rpackage{Rgraphviz} will dutifully plot
every edge it is told exists (and since the graph is 'undirected' they
both have no arrows).  This is easily remedied, and will be discussed
in the section 'Setting attributes via node and edge lists'.
...
In many cases, the user will not want to have the graph rendered
exactly as represented by the \Rpackage{graph} package.  For
instance, in the first example it was discussed how undirected graphs
will plot oddly by default in \Rpackage{Rgraphviz}, and this is
something that is easily fixed via this method.  The functions
...
it.  To remove the double edges, we need to make sure that we are only
reporting one of the two edges representing each proper edge and need
to remove the reciprocated edge from the edge list.

<<removeDoubles>>=
for (i in c("d~a", "b~a", "e~a", "h~a", "c~h", "h~b", "h~f", "f~e",
            "e~d", "d~b", "d~f", "d~h", "e~h", "b~e", "b~f", "f~a"))
    edges[[i]] <- NULL
@

It does not matter which edge you remove, however take note that if we
had just removed edge "a~b" we'd have lost the green edge as the "b~a"
edge was not colored in the example above.  To plot this graph, one
needs to perform a layout with \Rfunction{agopen} and then plot the
resulting \Robject{Ragraph} object:
...
In many of the remaining examples of this document, we will be
removing the double edges.
...
Note that using an attribute list is functionally equivalent to the
node and edge lists presented in the previous section.
...
sgLayout1 <- agopen(g1, "testSG1", edges=edges, subGList=list(sg1, sg2, sg3))
plot(sgLayout1)
@
...
sgLayout2 <- agopen(g1, "testSG2", edges=edges, subGList=list(sg1, sg2))
plot(sgLayout2)
@
...
pgEdges <- buildEdgeList(pieGraph)
for (i in c("31339_at~31340_at", "31339_at~31342_at",
	    "31339_at~31343_at", "31339_at~31344_at",
	    "31339_at~31346_at", "31340_at~31344_at",
	    "31340_at~31342_at", "31340_at~31343_at",
            "31340_at~31346_at", "31341_at~31346_at", 
	    "31342_at~31343_at", "31342_at~31344_at",
	    "31342_at~31346_at", "31343_at~31344_at",
            "31343_at~31346_at", "31344_at~31346_at"))
    pgEdges[[i]] <- NULL

## Do a layout of the graph, but don't plot it yet
pgLayout <- agopen(pieGraph, "foo", edges=pgEdges)
