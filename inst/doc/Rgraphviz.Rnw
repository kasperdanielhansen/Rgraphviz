...
\author{Jeff Gentry, contributions from Robert Gentleman, Wolfgang Huber}
\begin{document}
...
outdir=tempdir()
nd = nodes(IMCAGraph)
plotFiles = file.path(outdir, paste(seq(along=nd), 'png', sep='.'))
for(i in seq(along=nd)) {
  png(plotFiles[i], width=400, height=600)
