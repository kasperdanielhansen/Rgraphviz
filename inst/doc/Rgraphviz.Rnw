...
We prepare a graph with two subgraphs: one is hcs.gxl, the other is conn.gxl, 
both are from package \Rpackage{RBGL}.  We specify the 1st subgraph is NOT a 
cluster, while the 2nd one is.
...
conn <- fromGXL(file(system.file("XML/conn.gxl",package="RBGL")))
jg <- join(conn, hcs)
sgl <- vector(mode="list", length=2)
sgl[[1]] <- list(graph=hcs, cluster=FALSE)
sgl[[2]] <- list(graph=conn, cluster=TRUE)
ng <- LLagopen(jg, "tmpsg", subGList=sgl)
@

Note: this instance of \Robject{Ragraph} class from \Rfunction{LLagopen} 
...
TODO: a new function name for \Rfunction{LLagopen}?
...
<<GetDefAttrsDemo1>>=
getDefAttrsGraph(ng)
getDefAttrsCluster(ng, 1)
getDefAttrsNode(ng)
getDefAttrsEdge(ng)
getDefAttrs(ng)
@

The function \Rfunction{getDefAttrs} is just a shortform to call the first 3
functions.  

The return value, "ATTR\_NOT\_DEFINED", indicates that 
you haven't explicitly set any default attributes yet, 
...
<<SetDefAttrsDemo1>>=
setAttrsGraph(ng, attrname="bgcolor", attrval="red", defaultval="yellow")
setDefAttrsCluster(ng, 1, c("bgcolor"), c("blue"))
setDefAttrsNode(ng, c("fillcolor"), c("yellow"))
setDefAttrsEdge(ng, c("color"), c("blue"))
@

Again, if you like the notation from package \Robject{graph} better, the following
codes accomplish the same:

<<DataDefaultsDemo2>>=
graphDataDefaults(ng, c("size", "bgcolor")) <- c("1", "yellow")
clusterDataDefaults(ng, 1, c("bgcolor", "color")) <- c("blue", "orange")
...
<<getAttrsDemo1>>=
getAttrsGraph(ng, attrname="bgcolor")
getAttrsCluster(ng, 1, attrname="bgcolor")
getAttrsNode(ng, "A1", attrname="shape")
getAttrsEdge(ng, "B2", "B4", attrname="color")
@
...
clusterData(ng, 1, "bgcolor") 
nodeData(ng, "A", c("fontcolor", "width"))
edgeData(ng, "F", "G", c("color", "arrowhead"))
...
<<setAttrsDemo1>>=
setAttrsGraph(ng, attrname="bgcolor", attrval="red", defaultval="yellow")
setAttrsCluster(ng, 1, attrname="bgcolor", attrval="blue")
setAttrsNode(ng, "A", attrname="shape", attrval="rect", defaultval="circle")
setAttrsEdge(ng, "A", "C", attrname="color", attrval="purple")
@

<<DataDemo2>>=
graphData(ng, "bgcolor") <- "orange"
clusterData(ng, c(0, 1), "color") <- "red"
nodeData(ng, "A", c("fontcolor", "width")) <- c("blue", 0.5)
 
edgeData(ng, "F", "G", c("color", "style")) <- c("yellow", "dotted")
@
